...
public class TravelSpecification {
	 ...
	 */
	public static Specification<TravelEntity> searchByCriteres(final TravelCriteria criteres) {
		return new Specification<TravelEntity>() {
			@Override
			public Predicate toPredicate(Root<TravelEntity> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (null != criteres) {
					...
					if (null != criteres.getTotalTime()) {
						Predicate p = cb.equal(root.<Integer> get("totalTime"), criteres.getTotalTime());
						listeCond.add(p);
					}
